...
{
	float head_dmg_modifier = GetDamageReduction(HEAD_DMG);
	float torso_dmg_modifier = GetDamageReduction(TORSO_DMG);
	float legs_dmg_modifier = GetDamageReduction(LEGS_DMG);
	float arms_dmg_modifier = GetDamageReduction(ARMS_DMG);
	...
	if (0 != pev->takedamage)
	{
		...
		case HITGROUP_GENERIC:
			//Print("TAKING GENERIC DAMAGE!\n", print_chat);  // nMod
			//nMod.BodyGroupHealth[GENERIC_DMG]--;		   // nMod
			break;

		case HITGROUP_HEAD:
			//Print("TAKING HEAD DAMAGE!\n", print_chat); // nMod
			//nMod.BodyGroupHealth[HEAD_DMG]--;			// nMod
			flDamage *= gSkillData.plrHead;
			flDamage *= head_dmg_modifier;		       // nMod
			break;

		case HITGROUP_CHEST:
			//Print("TAKING CHEST DAMAGE!\n", print_chat); // nMod
			//nMod.BodyGroupHealth[CHEST_DMG]--;			 // nMod
			flDamage *= gSkillData.plrChest;
			flDamage *= torso_dmg_modifier;					// nMod
			break;

		case HITGROUP_STOMACH:
			//Print("TAKING STOMACH DAMAGE!\n", print_chat); // nMod
			//nMod.BodyGroupHealth[STOMACH_DMG]--;		   // nMod
			flDamage *= gSkillData.plrStomach;
			flDamage *= torso_dmg_modifier;					 // nMod
			break;

		case HITGROUP_LEFTARM:
			//Print("TAKING LEFT ARM DAMAGE!\n", print_chat); // nMod
			//nMod.BodyGroupHealth[LEFTARM_DMG]--;			// nMod
			//break; // nMod: was fallthru

		case HITGROUP_RIGHTARM:
			//Print("TAKING RIGHT ARM DAMAGE!\n", print_chat); // nMod
			//nMod.BodyGroupHealth[RIGHTARM_DMG]--;			 // nMod
			flDamage *= gSkillData.plrArm;
			flDamage *= arms_dmg_modifier;					 // nMod
			break;

		case HITGROUP_LEFTLEG:
			//Print("TAKING LEFT LEG DAMAGE!\n", print_chat); // nMod
			//nMod.BodyGroupHealth[LEFTLEG_DMG]--;			// nMod
			//break;  // nMod: was fallthru

		case HITGROUP_RIGHTLEG:
			//Print("TAKING RIGHT LEG DAMAGE!\n", print_chat); // nMod
			//nMod.BodyGroupHealth[RIGHTLEG_DMG]--;			 // nMod
			flDamage *= gSkillData.plrLeg;
			flDamage *= legs_dmg_modifier;					// nMod
			break;
...
bool CBasePlayer::TakeDamage(entvars_t* pevInflictor, entvars_t* pevAttacker, float flDamage, int bitsDamageType)
{
	...
	if (flDamage >= NANO_BLEED_DAMAGE && pev->armorvalue == 0) // nMod
	{
		if (!nMod.PlayerIsBleeding)
			nMod.PlayerIsBleeding = true;
	}
	...
	pev->punchangle.x = GetHitPunch(pev); // nMod
...
void CBasePlayer::Killed(entvars_t* pevAttacker, int iGib)
{
	...
	nMod.PlayerIsBleeding = false;  // nMod
	...
	{ // fully under water
		...
		if (pev->air_finished < gpGlobals->time) // drown!
		{
			if (pev->pain_finished < gpGlobals->time)
			{
				...
				// nMod: for swimming skill
				float dmg_modifier = GetDrownDamage();

				if (pev->dmg > dmg_modifier) // nMod: default was 5
					pev->dmg = dmg_modifier; // nMod: default was 5
...
void CBasePlayer::PreThink()
{
	...
	NanoThink(pev);  // nMod: move into PlayerThink() ?
...
void CBasePlayer::CheckTimeBasedDamage()
{
	...
	if (fabs(gpGlobals->time - m_tbdPrev) < 2.0)  // nMod - could change this for resistance skill
		return;
	...
	for (i = 0; i < CDMG_TIMEBASED; i++)
	{
		...
		if ((m_bitsDamageType & (DMG_PARALYZE << i)) != 0)
		{
			...
			case itbd_DrownRecover:
				...
				if (m_idrowndmg > m_idrownrestored)
				{
					int idif = V_min( m_idrowndmg - m_idrownrestored, GaspForAir() );  // nMod: default is increments of 10 health 
			...
			if (0 != m_rgbTimeBasedDamage[i])
			{
					...
					((i == itbd_Poison) && (m_rgbTimeBasedDamage[i] < POISON_DURATION)))
				{
					...
					if (0 != nMod.Inventory[ITEM_ANTIDOTE]) // nMod
					{
						m_rgbTimeBasedDamage[i] = 0;

						//m_rgItems[ITEM_ANTIDOTE]--;
						nMod.Inventory[ITEM_ANTIDOTE]--;   // nMod
...
void CBasePlayer::PostThink()
{
	...
	if ((FBitSet(pev->flags, FL_ONGROUND)) && (pev->health > 0) && m_flFallVelocity >= PLAYER_FALL_PUNCH_THRESHHOLD)
	{
		...
		{ // after this point, we start doing damage
			...
			if (flFallDamage > 0)
			{
				flFallDamage = GetFallDamage(flFallDamage);		// nMod: for legs skill
...
{
	if (0 == g_psv_cheats->value && iImpulse != NANO_CHEAT)  // nModtech cheat will work without sv_cheats enabled
	{
		return;
	}
	...
	case NANO_CHEAT:
		...
		// nMod: the weapon we will equip first
		GiveNamedItem("weapon_9mmAR");
