...
int CSqueakGrenade::Classify()
{
	...
	if (m_hEnemy != NULL)
	{
		...
		{
		case CLASS_PLAYER:
			//if (GetSkillLevel(SKILL_WEAPONS_ALIEN) > SKILL_BEGINNER)  // nMod
...
void CSqueakGrenade::HuntThink()
{
	...
	if (m_hEnemy == NULL || !m_hEnemy->IsAlive())
	{
		...
		// nMod TODO: ignore player at higher alien weapon skill
	}
...
bool CSqueak::GetItemInfo(ItemInfo* p)
{
	...
	p->pszAmmo1 = "Snarks";
	p->iMaxAmmo1 = SNARK_MAX_CARRY; //GetMaxCarry(WEAPON_SNARK, SKILL_WEAPONS_ALIEN); // nMod
	p->pszAmmo2 = NULL;
	p->iMaxAmmo2 = -1;
	p->iMaxClip = WEAPON_NOCLIP;
	p->iSlot = /*4;*/     GetWeaponMenuColumn(WEAPON_SNARK); // nMod
	p->iPosition = /*3;*/ GetWeaponMenuRow(WEAPON_SNARK); // nMod
