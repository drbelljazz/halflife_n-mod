...
void EV_FireMP5(event_args_t* args)
{
	...
	// nMod: Replaced default sounds with CS MP5 sound
...
void EV_FireDesertEagle(event_args_t* args)
{
	...
	gEngfuncs.pEventAPI->EV_PlaySound(idx, origin, CHAN_WEAPON, "weapons/cs_deagle.wav", gEngfuncs.pfnRandomFloat(0.92, 1), ATTN_NORM, 0, 98 + gEngfuncs.pfnRandomLong(0, 3));
...
void EV_PipeWrench(event_args_t* args)
{
	...
	if (iBigSwing)
	{
		...
		gEngfuncs.pEventAPI->EV_PlaySound(idx, origin, CHAN_WEAPON, "weapons/pwrench_big_miss.wav", gEngfuncs.pfnRandomFloat(0.92, 1), ATTN_NORM, 0, PITCH_NORM);
	...
	else
	{
		...
		else
		{
			...
			case 0:
				...
				gEngfuncs.pEventAPI->EV_PlaySound(idx, origin, CHAN_WEAPON, "weapons/pwrench_miss1.wav", gEngfuncs.pfnRandomFloat(0.92, 1), ATTN_NORM, 0, PITCH_NORM);
				break;
			case 1:
				//EV_PlaySound(idx, origin, CHAN_WEAPON, "weapons/pwrench_miss2.wav", 1, ATTN_NORM, 0, PITCH_NORM);
				gEngfuncs.pEventAPI->EV_PlaySound(idx, origin, CHAN_WEAPON, "weapons/pwrench_miss2.wav", gEngfuncs.pfnRandomFloat(0.92, 1), ATTN_NORM, 0, PITCH_NORM);
				break;
			}
		}
...
#if 0
//========================
//	PIPE WRENCH BIG SWING
//========================
int g_iClub2;

// Only predict the miss sounds, hit sounds are still played
// server side, so players don't get the wrong idea.
void EV_PipeWrenchBig(event_args_t* args)
{
	const int idx = args->entindex;
	Vector origin = args->origin;
	const bool iBigSwing = /*0 !=*/args->bparam1;
...
void EV_FireTMP(event_args_t* args)
{
	...
	if (EV_IsLocal(idx))
	{
		// Add muzzle flash to current weapon model
		EV_MuzzleFlash();
		gEngfuncs.pEventAPI->EV_WeaponAnimation(TMP_SHOOT1 + gEngfuncs.pfnRandomLong(0, 2), 0);
	...
	{
	case 0:
		gEngfuncs.pEventAPI->EV_PlaySound(idx, origin, CHAN_WEAPON, "weapons/tmp-1.wav", 1, ATTN_NORM, 0, 94 + gEngfuncs.pfnRandomLong(0, 0xf));
		break;
	case 1:
		gEngfuncs.pEventAPI->EV_PlaySound(idx, origin, CHAN_WEAPON, "weapons/tmp-2.wav", 1, ATTN_NORM, 0, 94 + gEngfuncs.pfnRandomLong(0, 0xf));
		break;
	}
...
void EV_FireAK47(event_args_t* args)
{
	...
	if (EV_IsLocal(idx))
	{
		...
		//if (args->ducking == 0)
		//{
			V_PunchAxis(0, -4.0); // nMod:  Client-side recoil??? - WIP
			//V_PunchAxis(0, GetWeaponRecoil(/*NULL,*/ WEAPON_AK47));  // nMod: attempt
		//}
		//else
		//	V_PunchAxis(0, 0.0); // nMod:  Client-side recoil - WIP
	}
	...
	EV_GetGunPosition(args, vecSrc, origin);
	VectorCopy(forward, vecAiming);
	EV_HLDM_FireBullets(idx, forward, right, up, 1, vecSrc, vecAiming, 8192, BULLET_PLAYER_MP5, 0, 0, args->fparam1, args->fparam2);
}
