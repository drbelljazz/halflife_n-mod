...
{
	return true;  // nMod: trying to get marines to see further
...
void CHGrunt::HandleAnimEvent(MonsterEvent_t* pEvent)
{
	...
	case HGRUNT_AE_DROP_GUN:
	{
		if (GetBodygroup(GUN_GROUP) != GUN_NONE)
		{
			...
			SetBodygroup(GUN_GROUP, GUN_NONE);	if (GetBodygroup(GUN_GROUP) != GUN_NONE)
		{
			Vector vecGunPos;
			Vector vecGunAngles;
...
void CHGrunt::Spawn()
{
	...
	pev->effects = 0;
	pev->health = 125;  // nMod: Tougher grunts
...
void CHGrunt::AlertSquad() // nMod
{
	// nMod: this is where we can attempt to alert the entire marine squad when one is killed
