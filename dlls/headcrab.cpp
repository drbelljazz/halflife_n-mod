...
{
	return CLASS_INSECT;  // was alien prey
}
...
void CHeadCrab::HandleAnimEvent(MonsterEvent_t* pEvent)
{
	...
	case HC_AE_JUMPATTACK:
		...
		CBaseMonster::HandleAnimEvent(pEvent); // nMod: No jump for now
		break;   // nMod: No jump for now
	{
...
{
	return false;  // nMod: temporary


	if (FBitSet(pev->flags, FL_ONGROUND) && flDist <= 256 && flDot >= 0.65)   // Checks distance from player
	{
		return true;
	}
	return false;
