...
void CHud::Init()
{
	...
	//#if 0
	// nMod: new HUD color
	CVAR_CREATE("hud_red", "200", FCVAR_ARCHIVE);
	CVAR_CREATE("hud_green", "200", FCVAR_ARCHIVE);
	CVAR_CREATE("hud_blue", "200", FCVAR_ARCHIVE);
	//#endif
...
void CHud::VidInit()
{
	RPGStats_Setup(); // Client-side NanoMod init
	//SetDefaultSkills(); // Client-side NanoMod init
	...
	if (!m_pSpriteList)
	{
		...
		//else
		//	m_pSpriteList = SPR_GetList("sprites/alt_hud/hud.txt", &m_iSpriteCountAllRes);
