...
void WeaponsResource::LoadWeaponSprites(WEAPON* pWeapon)
{
	...
	// nMod: option for minimal crosshair
	//int crosshair_mode = CVAR_GET_FLOAT("altcrosshair");

	//if (crosshair_mode == 0)
		sprintf(sz, "sprites/%s.txt", pWeapon->szName);
	//else
	//	sprintf(sz, "sprites/alt_hud/%s.txt", pWeapon->szName);
...
int DrawBar(int x, int y, int width, int height, float f)
{
	...
	//width /= 2;	 // nMod: shrinking ammo bar since we are using the smaller HUD sprites
	//height /= 2; // nMod: shrinking ammo bar since we are using the smaller HUD sprites
...
bool CHudAmmo::DrawWList(float flTime)
{
	...
	// nMod
	/*int result = (CVAR_GET_FLOAT("inventory_visible"));
	if (result > 0)
	//Print("SHOWING INVENTORY!\n", print_center);
	ConsolePrint("HIHIHIHIHIH");*/
	//
	...
	for (i = 0; i < MAX_WEAPON_SLOTS; i++)
	{
		...
		if (i == iActiveSlot)
		{
			...
			for (int iPos = 0; iPos < MAX_WEAPON_POSITIONS; iPos++)
			{
				...
				//DrawAmmoBar(p, x + giABWidth / 2, y, giABWidth, giABHeight);    // nMod: hide the ammo bar for now until we decide on a new HUD color
