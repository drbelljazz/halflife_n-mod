...
bool CGauss::GetItemInfo(ItemInfo* p)
{
	...
	p->pszAmmo1 = "uranium";
	p->iMaxAmmo1 = URANIUM_MAX_CARRY;  //GetMaxCarry(WEAPON_GAUSS, SKILL_WEAPONS_ENERGY); // nMod
	p->pszAmmo2 = NULL;
	p->iMaxAmmo2 = -1;
	p->iMaxClip = WEAPON_NOCLIP;
	p->iSlot = /*3;*/     GetWeaponMenuColumn(WEAPON_GAUSS);  // nMod
	p->iPosition = /*1;*/ GetWeaponMenuRow(WEAPON_GAUSS); // nMod
...
void CGauss::PrimaryAttack()
{
	...
	{
		// nMod: energy weapons in water kill you, just like QUAKE!
		m_pPlayer->pev->health = -666;
		EMIT_SOUND(ENT(pev), CHAN_BODY, "gib.wav", 1, ATTN_NORM);
		return;
	...
	m_pPlayer->m_flNextAttack = UTIL_WeaponTimeBase() + GetGaussRefire();   // nMod: gauss refire delay
}

void CGauss::SecondaryAttack()
{
	if (CVAR_GET_FLOAT("skill_energy") == SKILL_BEGINNER)
	{
		#ifndef CLIENT_DLL
			Print("NEED HIGHER ENERGY SKILL!", print_center);
		#endif
...
#endif
	m_pPlayer->pev->punchangle.x -= GetWeaponRecoil(/*m_pPlayer, */WEAPON_GAUSS); // nMod
