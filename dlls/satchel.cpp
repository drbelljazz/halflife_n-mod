...
bool CSatchel::GetItemInfo(ItemInfo* p)
{
	...
	p->iMaxAmmo1 = SATCHEL_MAX_CARRY; //GetMaxCarry(WEAPON_SATCHEL, SKILL_WEAPONS_EXPLOSIVE); // nMod
	p->pszAmmo2 = NULL;
	p->iMaxAmmo2 = -1;
	p->iMaxClip = WEAPON_NOCLIP;
	p->iSlot = /*4;*/     GetWeaponMenuColumn(WEAPON_SATCHEL);  // nMod
	p->iPosition = /*1;*/ GetWeaponMenuRow(WEAPON_SATCHEL); // nMod
...
void CSatchel::Throw()
{
	if (0 != m_pPlayer->m_rgAmmo[m_iPrimaryAmmoType])
	{
		...
		float strength = ThrowStrength(WEAPON_SATCHEL); // nMod

		Vector vecThrow = gpGlobals->v_forward * strength + m_pPlayer->pev->velocity;
