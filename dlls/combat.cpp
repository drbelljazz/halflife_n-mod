...
void CBaseMonster::GibMonster()
{
	...
	EMIT_SOUND(ENT(pev), CHAN_VOICE, "common/null.wav", 1, ATTN_NORM); // nMod: don't talk after death
	...
	if (HasHumanGibs())
	{
		if (CVAR_GET_FLOAT("violence_hgibs") != 0) // Only the player will ever get here
		{
			...
			CGib::SpawnRandomGibs(pev, CVAR_GET_FLOAT("gibcount"), true); // throw some human gibs.    // nMod: now using out CVAR gibcount
		}
		gibbed = true;
	}
	else if (HasAlienGibs())
	{
		...
		{
			CGib::SpawnRandomGibs(pev, CVAR_GET_FLOAT("gibcount"), false); // Throw alien gibs
		}
		gibbed = true;
	}
...
Activity CBaseMonster::GetDeathActivity()
{
	...
	case HITGROUP_HEAD:
		...
		if (1 == CVAR_GET_FLOAT("showaccuracy")/* && !pev->classname*/) // nMod
		{
			//bool yes;

			//if (g_pevLastInflictor != NULL)
			//	yes = FStrEq(STRING(g_pevLastInflictor->classname), "player");
			//else
			//	yes = false;
...
bool CBaseMonster::TakeDamage(entvars_t* pevInflictor, entvars_t* pevAttacker, float flDamage, int bitsDamageType)
{
	...
	// nMod
	if (pev->health <= 0 && !pev->deadflag)
		ScorePoints(pevAttacker, pev); // seems to be the best spot so far
	
	// nMod: check if we should allow death during scripted sequence
	if ( !KillSequence() ) 
	//
